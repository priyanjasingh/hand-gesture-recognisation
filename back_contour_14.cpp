	...
	for(int i = 0;i < frame.rows ;i++){
	  for(int j = 0;j < 3*(frame.cols) ;j++){
	    ...
	    */
	    int level=25;
	    int level2=15;
	    if((b-b1<=level && b-b1>=level2) || (b-b1 >=-level && b-b1<=-level2))ptr1[frame.step * i + j ]=5;
	      else ptr1[frame.step*i + j ]=b;   
	      
	    if((g-g1<=level && g-g1>=level2) || (g-g1<=-level2 && g-g1 >=-level))ptr1[frame.step * i + j+1 ]=5;
	       else ptr1[frame.step*i + j+1 ]=g;
	      
	    if((r-r1<=level && r-r1>=level2) || (r-r1<=-level2 && r-r1 >=-level))ptr1[frame.step * i + j+2 ]=5;
